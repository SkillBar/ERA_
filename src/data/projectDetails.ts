...
const DETAILS: Record<string, ProjectPageDetail> = {
  pilot: {
    ...
    shortDescription:
      "Гонки в реальном мире от первого лица с управлением, в котором участвуют элементы силы мысли. Level Up Games — это не краудфандинг: мы создаём новый формат гейминга на стыке BCI и симуляторов.",
    targetAudience:
    ...
    fullDescription: `
Level Up Games — это гоночный симулятор от первого лица, где управление дополняется элементами нейроинтерфейса (BCI): концентрация, внимание и состояние пилота влияют на геймплей. FPV-камеры установлены прямо в салоне машин, переднее стекло снято — вид как из кокпита без преград.
...
Команда работает над прототипом симулятора, интеграцией BCI-устройств и сценариями использования для киберспорта и развлечений.
    ...
    gameModes: [
      {
        title: "Город Конструктор",
        description:
          "Режим «Город Конструктор» — это гонки и дрифт в городе, собранном из модулей: трасса с поворотами, чекпоинты, здания и декорации можно менять и достраивать. Несколько машин одновременно участвуют в заезде: дрифт в поворотах, дым от колёс, вид от первого лица из салона. Игроки могут собирать конфигурации трасс под разные форматы — от спринта до выносливости — и соревноваться на своих конструкциях.",
        imageUrl: "/projects/pilot/city-constructor.png",
    ...
    coreTeamMembers: [
      {
        ...
        name: "Денис",
        role: "CEO",
        responsibilities: ["Менеджмент"],
        capTablePercent: 30,
      },
      {
        id: "ctm2",
        name: "Илья",
        role: "Отдел партнерства / Product Manager",
        responsibilities: ["Подключение биржевых организаций на карту"],
        capTablePercent: 25,
      },
      {
        id: "ctm3",
        ...
        role: "CTO",
        responsibilities: [
          "Знать электронику и выбирать лучшую",
          "Контроль разработки инфраструктуры",
        ],
        capTablePercent: 25,
      },
      {
        id: "ctm4",
        name: "Антон",
        role: "Руководитель по производству",
        responsibilities: ["Производство"],
        capTablePercent: 20,
      },
    ],
    teamMembers: [
      { id: "tm1", name: "Эльф", role: "Электронщик", bio: "Исполнитель по электронным компонентам и сборке." },
      { id: "tm2", name: "Никита", role: "Электронщик", bio: "Исполнитель по электронным компонентам и тестированию." },
    ],
    perks: [
      {
        title: "Ранний доступ",
    ...
    productsOrServicesDescription:
      "Level Up Games — гоночный симулятор от первого лица с элементами управления через BCI. Продукт: играбельные режимы (дрифт, офф-роад, формула, водные и воздушные гонки), прототип интеграции нейроинтерфейса, формат живых мероприятий с машинками и FPV.",
