...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-border bg-white dark:bg-background">
      <div className="container relative z-20 mx-auto flex h-20 items-center justify-between gap-2">
        ...
        {/* Поиск: прижатие к верху (items-start), чтобы выпадающий список рос только вниз и не перекрывал строку поиска; pt центрирует закрытый поиск по вертикали шапки */}
        <div className="absolute left-1/2 top-0 z-30 hidden h-full w-full max-w-xl -translate-x-1/2 items-start justify-center pt-[22px] md:flex [padding-inline:30px]">
          <SearchAutocomplete placeholder="Поиск проектов" className="w-full" variant="soft" size="header" />
        </div>
      ...
      {mobileMenuOpen && (
        <div className="border-t border-border bg-card/95 py-4 md:hidden [padding-inline:30px]">
